...
app.post('/login', function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (userFindByEmail.errorCode == ErrorsCode_1.ERROR_CODE.SUCCESSFULLY && userFindByEmail.data.email == email && userFindByEmail.data.password == password) {
            const user_id = userFindByEmail.data._id;
            const access_token = jwt.sign({ user_id, email }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: '1d' });
            const refresh_token = jwt.sign({ user_id, email }, process.env.REFRESH_TOKEN_SECRET);
